...
def main():
    ...
    tests_total += 1
    if test_query(
        "Exclusion: 'physics' --exclude 'mechanics'",
        query="physics",
        exclude="mechanics"
    ):
        tests_passed += 1
    ...
    print(f"\n{'='*80}")
    print(f"TEST: Error case - union with single query")
    print(f"{'='*80}")
    tests_total += 1
    try:
        result = build_boolean_query(query="single query", union=True)
