...
for i, r in enumerate(results_summary, 1):
    ...
    if 'error' in r:
        ...
        print(f"   Max: {r['max_score']:.4f} | Avg: {r['avg_score']:.4f} | Min: {r['min_score']:.4f}")
...
all_high = [r for r in results_summary if r['expected'] in ['HIGH', 'MEDIUM-HIGH'] and 'error' not in r]
